...
- **Unexplained charts** — every chart must be followed by a markdown cell that explains what the chart shows in plain language. Call out the key takeaway, notable outliers, and any surprises. The reader should never have to guess what they're looking at. Example: "Magnesium stands out at 89% positive (n=56), while SSRIs are the only treatment with a net negative signal (32% positive, n=116). Most treatments cluster between 50–70% positive."
...
- **Inaccessible language** — the audience is patients AND researchers, not just statisticians. Every statistical result must be presented twice: once with the formal test (p-value, CI, test name) for scientific rigor, and once in plain language a patient can act on. Use the format: `**What this means:** 72% of people who tried LDN reported positive outcomes — significantly better than chance (p=0.002). This is one of the stronger signals in the dataset.` Never present a p-value without explaining what it means in context.
- **No recommendation** — every notebook must end with a **ranked recommendation section** that translates the statistics into actionable guidance. Group treatments into tiers based on evidence strength (sample size + statistical significance): **Strong evidence** (n>=30, p<0.05), **Moderate evidence** (n>=15 or p<0.10), **Preliminary** (n<15, not significant). Include a one-line plain-language note for each. Example: `**Strong evidence:** LDN — 72% positive (n=79, p=0.002). Most-discussed treatment for this symptom with consistently positive reports.`
- **Generic treatment names** — filter out non-actionable terms like "supplements", "medication", "treatment", "therapy" from results tables and charts. These are categories, not treatments a patient can look up or ask their doctor about.
- **Causal-context contamination** — in patient communities, some treatments appear in the data primarily because they *caused* the condition or *failed*, not because they were used for recovery. Examples: SSRIs in r/PSSD (caused the condition), birth control in r/abortion (failed to prevent pregnancy), the disease-defining drug in any adverse-reaction community. When analyzing "what helps recovery," these entries reflect the reason the person is in the community, not the drug's efficacy for the question being asked. During the Data Exploration step, identify treatments whose sentiment is likely dominated by causal/failure context rather than treatment response. **Filter these from recovery/recommendation analyses and explain why.** In the notebook, add a brief note: "Birth control (11% positive) is excluded from recovery recommendations — negative sentiment reflects contraceptive failure (why users are on r/abortion), not post-procedure recovery outcomes." The same logic applies to any community where a specific drug class is the reason the community exists (e.g., SSRIs in PSSD, vaccines in adverse-reaction communities).
