...
Each extractor wraps one of the executable scripts in ``scripts/`` via a subprocess
...
class ExtractorError(RuntimeError):
    ...
    def __init__(self, extractor: str, returncode: int, detail: str | None = None) -> None:
        msg = f"{extractor} failed with exit code {returncode}."
        if detail:
            msg += f"\n{detail}"
        super().__init__(msg)
...
class BaseExtractor(ABC):
    ...
    #: Filename of the executable script in ``scripts/`` that this extractor delegates
    ...
    def __init__(
        ...
        # Resolve the script path relative to the patientpunk package dir
        # (patientpunk/scripts/), so it works whether the package is installed
        # as a wheel or run from a source checkout.
        self._script_path: Path = Path(__file__).resolve().parent.parent / "scripts" / self._SCRIPT
    ...
    def run(
        ...
        if not self._script_path.exists():
            ...
            result = ExtractorResult(
                ...
                stdout=None,
                stderr=(
                    f"Extractor script not found: {self._script_path}\n"
                    f"The patientpunk package looks installed incorrectly: the "
                    f"extraction scripts must ship under patientpunk/scripts/."
                ),
            )
        else:
            try:
