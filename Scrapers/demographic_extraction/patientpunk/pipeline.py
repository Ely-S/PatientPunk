...
from ._utils import clean_temp_dir, csv_fill_rate, find_newest_glob, get_schema_id
...
class Pipeline:
    ...
    def _run_phase_4(self) -> PhaseResult:
        ...
        disc = find_newest_glob(self._temp_dir, "discovered_records_*.json")
        if disc:
    ...
    def _collect_stats(self, phase: int) -> dict:
        ...
        if phase == 3:
            disc_path = find_newest_glob(td, "discovered_records_*.json")
            if not disc_path:
                return {}
            try:
