...
def run_pipeline(config: PipelineConfig, *, skip_extract: bool = False, skip_canonicalize: bool = False, skip_prefilter: bool = False) -> None:
    ...
    if not skip_canonicalize:
        ...
        mention_entries = json.loads(config.path(TAGGED_MENTIONS).read_text(encoding="utf-8"))
        all_drugs = {d for e in mention_entries for d in e.get("drugs_direct", []) + e.get("drugs_context", []) if d.strip()}
