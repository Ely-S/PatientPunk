...
def run_classification(
    ...
    *before* the `signal != "n/a"` writer gate, tagged with status
    ...
    real-neutral and parse-failure rates the DB throws away (prereq P2) — any
    analysis that reads the DB alone measures the gate, not the model.
    """
        ...
        while pending:
            ...
            for (entry, drug), result in zip(batch, results):
                if audit_sink is not None:
                    status = (
                        "parse_failure" if result.parse_failed
                    ...
                    audit_sink.append({
                        ...
                        "side_effects": result.side_effects, "status": status,
                    })
