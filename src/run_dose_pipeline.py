...
context) to the model, and writes report_doses rows under a new extraction_runs row. Runs
append: earlier rows stay, and the report_doses_latest view shows each report's most recent run.
Without --exclude-compound / --exclude-file the exclusions recorded by the sentiment run are
used. Cached model replies make reruns free.
