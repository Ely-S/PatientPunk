#!/usr/bin/env python3
"""
run_effects_pipeline.py — Extract every effect the author says the drug had on them, one row per effect per report.

Run after run_sentiment_pipeline.py and run_dose_pipeline.py with the same --db and --drug.
Reads the latest treatment report per post for that drug, sends each report (with its
parent post, the thread title and the report's dose rows as context) to the model, and
writes report_effects under a new extraction_runs row. An effect the author ties to a stated
dose points at its report_doses row. Re-running replaces a report's rows; cached model
replies make reruns free.
...
def _lines(path: str, what: str, parser: argparse.ArgumentParser) -> list[str]:
    values = [line.strip() for line in Path(path).read_text(encoding="utf-8").splitlines() if line.strip()]
    if not values:
        parser.error(f"{what} {path} contains no non-blank lines")
    return values
