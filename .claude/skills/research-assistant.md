...
**Null and negative findings are real findings.** Report them honestly and give them appropriate weight. "This widely-recommended treatment performs no better than chance" saves patients time and false hope. "No significant difference between these groups" tells a researcher where not to look next. Never spin a null into a positive, never bury non-significant results, never omit treatments because they performed poorly. Include them in the narrative, the recommendations (as "not recommended" or "insufficient evidence"), and the conclusion.
...
- **Qualitative evidence** — after the quantitative analysis, add a "What patients are saying" section. Query `posts.body_text` joined with `posts.post_date` for users who reported on the top treatments. Pull 3-5 quotes that illustrate the key finding. Quotes should be **long enough to tell a micro-story** (1-2 sentences, up to 40 words) — not truncated fragments that cut off mid-thought. Include the date: "One user reported (March 2026): '[Treatment] completely resolved my [symptom] within two weeks of starting. I went from barely functioning to back at work.'" Include at least one quote that contradicts or complicates the main narrative. Never use usernames. **Every quote must be self-contained — a reader should understand it without seeing the original thread.** If the quote contains pronouns ("both", "it", "this") that are ambiguous without context, either pick a different quote or add a bracketed clarification: "I have been on both [Drug A and Drug B] and unfortunately it did not solve it [my symptom]." If you can't determine what the pronouns refer to from the post text, don't use that quote. Do not include meta-commentary ("amazing stack — similar to mine"), social validation ("sending you love and support"), general emotional statements, or truncated fragments that end in "..." mid-sentence. If a quote is interesting but too long, paraphrase the context and quote the key clause. **Quotes must demonstrate the claim made by their category header.** If the header says "On support making the difference," the quote must show support CAUSING a better outcome ("My partner stayed with me the whole time and it made the pain manageable"), not just describe support existing ("My parents were supportive"). A quote that describes a fact without connecting it to an outcome does not support a causal claim. **Verify the sentiment label matches the quote content.** If a quote describes reconsidering or being open to a treatment, labeling it "negative" is misleading. If the quote's tone is ambiguous, either pick a clearer quote or label it "mixed" with a brief explanation.

**If you cannot find quotes that directly demonstrate a claim, omit the quotes section for that claim entirely.** Three vaguely related quotes are worse than no quotes. A quote about preparing hot water bottles does not demonstrate that support improves outcomes. A quote from someone offering sympathy does not show the recipient benefiting from it. The bar is: does the quote contain both the cause (the thing being claimed) AND the effect (the outcome it produced)? If not, skip it.
