...
See **`STATS_REFERENCE.md`** alongside this file for the full catalog of recommended statistical tests, which library function implements each (scipy, statsmodels, lifelines, etc.), and the specific warnings to emit per test with their severity tiers. The skill writes library calls inline in the notebook — no wrapper modules.

Every test result should carry structured warnings tagged by severity: `caveat` (minor), `caution` (interpret carefully), `unreliable` (do not trust). Surface `caution` and `unreliable` warnings to the reader as markdown callouts; `caveat` can stay inline in the stats table. The full warning code inventory and thresholds (e.g., `small_sample` when `n < 20`, `low_epp` when events-per-predictor < 10) lives in `STATS_REFERENCE.md`.

Standard library stack assumed available in the notebook kernel: `scipy`, `statsmodels`, `pandas`, `numpy`, `matplotlib`, `seaborn`. Optional libraries (`pingouin`, `lifelines`, `causalinference`) unlock additional tests — if missing, skip those analyses and note why in the plan rather than failing the build.
...
```python
# After placing text labels, check for overlaps and nudge them apart.
# Preferred: use adjustText (from adjustText import adjust_text; adjust_text(list(ax.texts)))
# which handles coordinate systems, arrow-drawing, and collision resolution.
#
...
for i, t1 in enumerate(texts):
    ...
    for t2 in texts[i + 1:]:
        ...
        if bb1.overlaps(bb2):
            # Nudge t2 upward by ~1.2 line heights, measured in pixels then
            # converted to the axes' data coordinate system
            nudge_px = t2.get_fontsize() * 1.2
            (_, y_origin_data) = inv_transform.transform((0, 0))
            (_, y_nudge_data) = inv_transform.transform((0, nudge_px))
            dy = y_nudge_data - y_origin_data
            x, y = t2.get_position()
            t2.set_position((x, y + dy))
```
