...
sample was drawn. Two of the pilot artifacts are **not tracked in git** —
`coding_input.csv` holds verbatim patient text and `ai_labels.csv` is the
analyst-only answer key — so they live in gitignored `data/irr_pilot/` and are
distributed to coders privately (and via S3), alongside the α reports. Both are
regenerable from the fixed seed (see Reproducibility below). The codebook and
blank template are tracked in this folder.

- `coding_input.csv` *(not tracked)* — the 300 sampled units, distributed to
