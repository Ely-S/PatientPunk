...
was drawn. Two of the pilot artifacts are **not tracked in git** —
`coding_input.csv` holds verbatim patient text and `ai_labels.csv` is the
analyst-only answer key — so they live in gitignored `data/irr_pilot_500/` and
are distributed to coders privately (and via S3). Both are regenerable from the
fixed seed (see Reproducibility below). The blank template is tracked in this
folder; the codebook is tracked in the 300-pilot folder.

- `coding_input.csv` *(not tracked)* — the 500 sampled units, distributed to
  coders (one row per sample, columns: sample_id, subreddit, post_date,
  unit_type, title, parent_context, post_text)
- `ai_labels.csv` *(not tracked)* — analyst-only file with stratum labels and
  AI-pipeline output for each sample (columns: sample_id, source_post_sha256,
  stratum, ai_drug_count, ai_drugs, ai_sentiments, ai_signal_strengths,
  keyword_match). `source_post_sha256` is a one-way SHA-256 hash of the source
  Reddit post/comment id — a stable opaque key, not resolvable back to the
  original post.
