...
def judge_post(client, judge, items) -> dict:
    """items: list of (field, gold_list, model_list). Returns {field: verdict}."""
    ...
    try:
        obj = parse_json_object(_bounded_call(client, judge, PROMPT.format(block=block),
                                              max_tokens=30 * len(items) + 400))
        out = {}
        for v in obj.get("verdicts", []):
            f = str(v.get("field", "")).strip()
            vv = str(v.get("verdict", "")).strip().lower()
            if f:
                out[f] = vv if vv in ("equivalent", "model_subset", "different") else "different"
        return out
    except Exception:
        return {}
...
def main():
    ...
    def run_one(t):
        ...
        out = []
        for f, g, mv in items:
            rec = {"model": m, "sample_id": s, "field": f, "verdict": verds.get(f, "different"),
                   "gold": g, "model_val": mv}
            out.append(rec)
