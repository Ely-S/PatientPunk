...
def main():
    ...
    lines.append("the analysis DB and figures. We have **not** run a quantitative")
    lines.append("sensitivity analysis on individual flagged aliases (no per-alias")
    lines.append("ablation table is checked in), so this file should not be read as")
    lines.append("evidence that the headline numbers are robust to those choices —")
    lines.append("only that the choices themselves are documented and inspectable.")
    lines.append("Producing a per-drug sensitivity table (drop each flagged alias,")
    lines.append("re-run, report shifted n / pos / pos_pct) is the open methodology")
    lines.append("task before publication.")
    lines.append("")
    ...
    lines.append("```")
    lines.append(
        f"python scripts/dump_drug_aliases.py "
        f"--db {args.db.as_posix()} --out {args.out.as_posix()}"
    )
    lines.append("```")
