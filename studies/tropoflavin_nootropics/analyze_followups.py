...
for r in csv.DictReader(open("source_B/records.csv", encoding="utf-8")):
    for entry in (r.get("treatment_outcome") or "").split("|"):
        ...
        if not which or outcome not in RANK: continue
        prev = author_out[which].get(r["author_hash"])
        if prev is None or RANK[outcome] > RANK[prev]:
            author_out[which][r["author_hash"]] = outcome

print("="*70); print("1. NO_EFFECT GAP — one vote per author per compound"); print("="*70)
tab={}
for k in ("7,8-DHF","4'-DMA"):
    c=collections.Counter(author_out[k].values()); n=sum(c.values())
    tab[k]=(c,n)
    ne=c["no_effect"]; lo,hi=wilson(ne,n)
    print(f"\n  {k}: n={n} authors  {dict(c)}")
    print(f"    no_effect {ne}/{n} = {100*ne/n:.1f}%  [{100*lo:.1f}, {100*hi:.1f}]")
(c1,n1),(c2,n2)=tab["7,8-DHF"],tab["4'-DMA"]
for label,key in (("no_effect vs rest","no_effect"),("helped vs rest","helped"),
                  ("worsened vs rest","worsened")):
    a,b=c1[key],n1-c1[key]; cc,d=c2[key],n2-c2[key]
    odds,p=fisher_exact([[a,b],[cc,d]])
    print(f"\n  {label:20s} 7,8-DHF {a}/{n1}={100*a/n1:.1f}%   4'-DMA {cc}/{n2}={100*cc/n2:.1f}%"
          f"   OR={odds:.2f}  Fisher p={p:.4f}")
...
print("\n"+"="*70); print("2. SENTIMENT BY USE-CASE (pipeline A, one vote per user per category)")
print("="*70)
res=[]
for cat,pat in PURPOSE.items():
    rx=re.compile(pat,re.I); best={}
    for r in sorted(rows,key=lambda r:(r["post_date"] or 0, SIG.get(r["sig"],0)),reverse=True):
        t=((r["title"] or "")+" "+(r["body_text"] or ""))
        if rx.search(t): best.setdefault(r["user_id"], r["sentiment"])
    n=len(best)
    if not n: continue
...
base_n=len({r["user_id"] for r in rows}); base_pos=None
bb={}
for r in sorted(rows,key=lambda r:(r["post_date"] or 0, SIG.get(r["sig"],0)),reverse=True):
...
for cat,n,pos,neg,pct,lo,hi in sorted(res,key=lambda x:-x[1]):
    ...
    print(f"  {cat:24s}{n:>7}{pos:>6}{neg:>6}{pct:>7.1f}%   [{lo:4.1f},{hi:5.1f}]{flag}")
print(f"  {'ALL (baseline)':24s}{base_n:>7}{base_pos:>6}{base_n-base_pos:>6}"
      f"{100*base_pos/base_n:>7.1f}%")
ps=[]
for cat,n,pos,neg,pct,lo,hi in res:
    odds,p=fisher_exact([[pos,n-pos],[base_pos-pos,(base_n-n)-(base_pos-pos)]])
...
for (cat,n,pct,p),q in sorted(zip(ps,adj),key=lambda x:x[1]):
    print(f"    {cat:24s} n={n:<4} {pct:5.1f}%   p={p:.3f}  q={q:.3f}"
          + ("  <-- survives" if q<0.05 else ""))

# ── 3. side effects ────────────────────────────────────────────────────────
print("\n"+"="*70); print("3. SIDE EFFECTS"); print("="*70)
se=collections.Counter(); n_with=0
for r in rows:
    raw=r["side_effects"]
    ...
    if not lst: continue
    n_with+=1
    for s in lst: se[str(s).strip().lower()]+=1
